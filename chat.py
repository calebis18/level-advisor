...
def get_db():
	...
	if db is None:
		if DATABASE_URL:
			if psycopg is None:
				raise RuntimeError('PostgreSQL support is missing. Install the requirements.txt dependencies.')
			db = g._database = psycopg.connect(
				DATABASE_URL, row_factory=dict_row, prepare_threshold=None
			)
			g._database_is_postgres = True
		else:
			db = g._database = sqlite3.connect(DATABASE)
			db.row_factory = sqlite3.Row
			g._database_is_postgres = False
	return db


def execute(db, statement, parameters=()):
	"""Run SQL on SQLite locally or Postgres/Supabase in deployment."""
	if getattr(g, '_database_is_postgres', False):
...
def init_db():
	with app.app_context():
		...
		if columns == set() and execute(db,
			"SELECT name FROM sqlite_master WHERE type='table' AND name='users_legacy'"
		).fetchone():
			for legacy_user in execute(db, 'SELECT username, password FROM users_legacy').fetchall():
				execute(db,
					'''INSERT INTO users
			...
			execute(db, 'DROP TABLE users_legacy')
		db.commit()
...
def current_user():
	if 'user_id' not in session:
		return None
	db = get_db()
	return execute(db, 'SELECT * FROM users WHERE id = ?', (session['user_id'],)).fetchone()
...
def api_register():
	...
	user = execute(db, 'SELECT * FROM users WHERE id = ?', (user_id,)).fetchone()
	session.clear()
...
def login():
	...
	if request.method == 'POST':
		...
		password = request.form['password']
		db = get_db()
		user = execute(db, 'SELECT * FROM users WHERE matric_number = ? OR email = ?', (username.upper(), username.lower())).fetchone()
